...
# dt: stable timestep (h>0 only)
    ...
    lam1 = abs(U[1,:]/U[0,:] - np.sqrt(cc2*beta + g*U[0,:])) #array
    lam2 = abs(U[1,:]/U[0,:] + np.sqrt(cc2*beta + g*U[0,:])) #array
    denom = np.maximum(lam1,lam2) #array

    dt = cfl*min(Kk/denom) #number
...
def step_forward_topog(U,B,dt,Nk,Kk):
    ### INPUT ARGS:
    # U: array of variarible values at t, size (Neq,Nk)
    # B: bottom topography
    # dt: stable time step
...
#    Sb[1,:] = 0.5*g*(Uminus[0,1:]**2 - Uplus[0,:-1]**2)
    ...
    # Raphi: Consider using this time step
#    # non-neg time-step:
...
#    dt = cfl_fc*dt
    ...
    if BC == 1: #PERIODIC
        ...
        # apply relaxation of velocity towards initial state on boundaries
#        relaxation = 10.
#        UU[1,0] = UU[1,0] - dt*relaxation*(UU[1,0]/UU[0,0] - 1.)
#        UU[1,Nk-1] = UU[1,Nk-1] - dt*relaxation*(UU[1,Nk-1]/UU[0,Nk-1] - 1.)
        # simply set boundary hu to 1
#        UU[1,0] = UU[0,0]
#        UU[1,-1] = UU[0,-1]
